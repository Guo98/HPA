...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    if (l && l->canBeRead()) {
        readHit.inc();
		if(std::find(currentCache.begin(), currentCache.end(), tag) == currentCache.end()) {
			if(currentCache.size() == numLines) {
				currentCache.pop_front();
			}
			currentCache.push_back(tag);
		} else {
			currentCache.remove(tag);
			currentCache.push_back(tag);
		}
#ifdef SESC_ENERGY
        rdEnergy[0]->inc();
	...
	// check if its a conf miss
	if(!compulsoryMiss && !capacityMiss) {
		confMiss.inc();
	}
