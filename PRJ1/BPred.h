...
private:
protected:
    int32_t correctPreds[3000];
    int32_t missPreds[3000];
    int32_t instInfo[3000];
    int32_t totalPreds[3000];
public:
...
public:
	...
	for(int i = 0; i < 3000; i++) {
		if(instInfo[i] == inst->currentID()) {
			// std::cout << "instruction was found " << instInfo[i] << "\n";
			found = true;
			index = i;
			break;
		}
