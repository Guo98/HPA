    ...
    , writeCoheMiss("%s:writeCoheMiss", name)
{
    ...
    cacheSize = SescConf->getInt(section, "size");
...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    // PROJECT3
    //if (invalidTagMap.find(tag) != invalidTagMap.end() && !l->isValid()) {
    if (!(checkCoherenceMiss(addr) == 0)) {
        // It is a coherence read miss
        readCoheMiss.inc();
    } else {
	    // check if its a comp miss
	    if(cacheTagHistory.find(tag) == cacheTagHistory.end()) {
		    readCompMiss.inc();
		    cacheTagHistory.insert({ tag, 1 });
	    } else {
			readReplMiss.inc();
	    }
	}
...
void SMPCache::doWrite(MemRequest *mreq)
{
	...
	if (!(checkCoherenceMiss(addr) == 0) || l) {
		writeCoheMiss.inc();
	} else {
		if(cacheTagHistory.find(tag) == cacheTagHistory.end()) {
			writeCompMiss.inc();
...
void SMPCache::realInvalidate(PAddr addr, ushort size, bool writeBack)
{
    while(size) {
        ...
        if (l) {
            ...
            invalidTagMap.insert(l->getTag());
            
            l->invalidate();
            
            // PROJECT3
            // If we get here, we know that the line is invalid
            // Then we check...
            //   1) Is the line (*l) in the cache currently?
            //   2) Check our new map to determine if the tag we were looking for *was* at the given line number? (lookup in invalidTagMap)
            //      - If so, then we have a read coherence miss
        }
