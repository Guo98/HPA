...
public:
    ...
    virtual CacheLine *checkCoherenceMissPrivate(Addr_t addr)=0;  // PROJECT3
protected:
...
public:
    ...
    // PROJECT3
    CacheLine *checkCoherenceMiss(Addr_t addr) {
        // 1) Do the logic from findLinePrivate()
        // 2) Check the found line to determine:
        /*
            a) the line is invalid
            b) the line USED to be valid
            c) the prevTag is equal to calcTag(addr)
        */
        
        /*std::cout << "CacheCore::checkCoherenceMiss";
        CacheLine *line = findLine(addr);
        Addr_t tagToFind = calcTag(addr);
        if (!line->isValid() && line->getPrevTag() && line->getPrevTag() == tagToFind) {
            return true;
        } else {
            return false;
        }*/
        return checkCoherenceMissPrivate(addr);
    }
...
protected:
    ...
    Line *checkCoherenceMissPrivate(Addr_t addr);  // PROJECT3
public:
    virtual ~CacheAssoc() {
        delete [] content;
        delete [] mem;
