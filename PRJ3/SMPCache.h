...
protected:
    ...
    std::map<PAddr, int> cacheTagHistory;
    // std::map<Paddr, Addr_t> invalidTagMap;  // For PROJECT3 I/J
    std::set<PAddr> invalidTagMap; // For PROJECT3 I/J
    std::list<PAddr> currentCache;
    int cacheSize;
...
public:
    ...
    // PROJECT3
    bool checkCoherenceMiss(PAddr addr) {
        return cache->checkCoherenceMiss(addr);
    }
