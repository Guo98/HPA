...
// PROJECT3
template<class State, class Addr_t, bool Energy>
typename CacheAssoc<State, Addr_t, Energy>::Line *CacheAssoc<State, Addr_t, Energy>::checkCoherenceMissPrivate(Addr_t addr)
{
    Addr_t tag = calcTag(addr);
    ...
    // Check most typical case
    if ((*theSet)->getPrevTag() == tag && !(*theSet)->isValid()) {
        return (*theSet);
    }
...
// PROJECT3
template<class State, class Addr_t, bool Energy>
typename CacheDM<State, Addr_t, Energy>::Line *CacheDM<State, Addr_t, Energy>::checkCoherenceMissPrivate(Addr_t addr)
{
    return 0;
}
...
// PROJECT3
template<class State, class Addr_t, bool Energy>
typename CacheDMSkew<State, Addr_t, Energy>::Line *CacheDMSkew<State, Addr_t, Energy>::checkCoherenceMissPrivate(Addr_t addr)
{
    return 0;
}
